...
cli.name('skycloudctl').version(version).description('controll utility for skycloud');

cli
  .command('read-config')
  .option('-f, --format', 'whether or not to format the json')
  .summary('output the configuration as json, exactly how SkyCloud sees it')
  .action(readConfig);
...
cli
  ...
  .argument('<directory>', 'the directory to import into SkyCloud')
  .action(importDir);
