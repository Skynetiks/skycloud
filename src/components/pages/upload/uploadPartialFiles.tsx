...
export async function uploadPartialFiles(
  ...
  for (let i = 0; i !== files.length; ++i) {
    ...
    for (let j = 0; j !== nChunks; ++j) {
      ...
      options.deletesAt !== 'never' && req.setRequestHeader('x-skycloud-deletes-at', options.deletesAt);
      options.format !== 'default' && req.setRequestHeader('x-skycloud-format', options.format);
      options.imageCompressionPercent &&
        req.setRequestHeader(
          'x-skycloud-image-compression-percent',
          options.imageCompressionPercent.toString(),
        );
      options.maxViews && req.setRequestHeader('x-skycloud-max-views', options.maxViews.toString());
      options.addOriginalName && req.setRequestHeader('x-skycloud-original-name', 'true');
      options.overrides_returnDomain &&
        req.setRequestHeader('x-skycloud-domain', options.overrides_returnDomain);

      ephemeral.password && req.setRequestHeader('x-skycloud-password', ephemeral.password);
      ephemeral.filename &&
        req.setRequestHeader('x-skycloud-filename', encodeURIComponent(ephemeral.filename));

      if (folder) {
        req.setRequestHeader('x-skycloud-folder', folder);
      } else if (ephemeral.folderId) {
        req.setRequestHeader('x-skycloud-folder', ephemeral.folderId);
      }

      identifier && req.setRequestHeader('x-skycloud-p-identifier', identifier);
      req.setRequestHeader('x-skycloud-p-filename', encodeURIComponent(file.name));
      req.setRequestHeader('x-skycloud-p-lastchunk', j === chunks.length - 1 ? 'true' : 'false');
      req.setRequestHeader('x-skycloud-p-content-type', file.type);
      req.setRequestHeader('x-skycloud-p-content-length', file.size.toString());
