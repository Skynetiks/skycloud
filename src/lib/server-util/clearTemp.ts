...
export async function clearTemp(): Promise<string> {
  ...
  if (files.length === 0) {
    logger.info('no temporary skycloud files found');
    return 'No temporary skycloud files found, so no files were cleared.';
  }
  ...
  return `Cleared ${files.length} temporary skycloud files.`;
}
