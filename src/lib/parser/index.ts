...
export type ParseValue = {
  ...
  metricsSkyCloud?: ParseValueMetrics;
...
export function parseString(str: string, value: ParseValue) {
  ...
  const data = {
    ...
    metricsSkyCloud: value.metricsSkyCloud,
  };
  ...
  const re =
    /\{(?<type>file|url|user|debug|link|metricsUser|metricsSkyCloud)\.(?<prop>\w+)(::(?<mod>(\w+|<|<=|=|>=|>|\^|\$|~|\/)+))?((::(?<mod_tzlocale>\S+?))|(?<mod_check>\[(?<mod_check_true>".*?")\|\|(?<mod_check_false>".*?")\]))?\}/gi;
