...
declare global {
  ...
  namespace NodeJS {
    interface ProcessEnv {
      SKYCLOUD_BUILD?: string;
      SKYCLOUD_DB_LOG?: string;
      SKYCLOUD_OVERRIDE_DISABLED_WORKER_LOG?: string;
    }
  }
}
...
export const schema = z.object({
  ...
  features: z.object({
    ...
    versionAPI: z.string().url().default('https://skycloud-version.diced.sh/'),
  }),
  invites: z.object({
    enabled: z.boolean().default(true),
  ...
  website: z.object({
    title: z.string().default('SkyCloud'),
    ...
    externalLinks: z
      ...
      .default([
        {
          name: 'Skynetiks Technologies',
          url: 'https://skynetiks.com',
        },
        {
          name: 'Contact Us',
          url: 'https://skynetiks.com/contact',
        },
      ]),
  ...
  mfa: z.object({
    totp: z.object({
      ...
      issuer: z.string().default('SkyCloud'),
    }),
  ...
  pwa: z.object({
    enabled: z.boolean().default(true),
    title: z.string().default('SkyCloud'),
    shortName: z.string().default('SkyCloud'),
    description: z.string().default('SkyCloud'),
...
export function validateConfigObject(env: ParsedConfig): Config {
  const building = !!process.env.SKYCLOUD_BUILD;
