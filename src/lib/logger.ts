...
export default class Logger {
  ...
  private isSkyCloudDebug(): boolean {
    const debugVar = process.env.DEBUG;
    if (!debugVar) return false;

    if (debugVar === 'skycloud') return true;

    const parts = debugVar.split(',').map((v) => v.trim());
    if (parts.includes('skycloud') || parts.includes('*')) return true;
  ...
  private write(message: string, level: LoggerLevel, extra?: Record<string, unknown>) {
    if (!isMainThread && this.workerDisabled() && !process.env.SKYCLOUD_OVERRIDE_DISABLED_WORKER_LOG) return;
  ...
  public debug(args: string, extra?: Record<string, unknown>) {
    if (!this.isSkyCloudDebug()) return this;
