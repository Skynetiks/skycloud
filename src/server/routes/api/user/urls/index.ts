  ...
  (server, _, done) => {
    ...
    server.post<{ Body: Body; Headers: Headers }>(
      ...
      async (req, res) => {
        ...
        const noJson = !!req.headers['x-skycloud-no-json'];
        ...
        const password = req.headers['x-skycloud-password']
          ? await hashPassword(req.headers['x-skycloud-password'])
          : undefined;
